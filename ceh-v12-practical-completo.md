...
```bash
java -jar <ruta>/openstego.jar <comando> [opciones]
```
...
|--------------|--------------|----------------------------------------------------------|
| embed        | --embed      | Insertar mensaje en archivo cover                       |
| extract      | --extract    | Extraer mensaje de archivo stego                         |
| gensig       | --gensig     | Generar archivo de firma para watermarking               |
| embedmark    | --embedmark  | Insertar watermark usando firma                          |
| checkmark    | --checkmark  | Verificar watermark en archivo                           |
| algorithms   | --algorithms | Listar algoritmos de esteganografía soportados           |
| readformats  | --readformats| Listar formatos soportados para cover file               |
| writeformats | --writeformats| Listar formatos soportados para stego file               |
...
| Opción                 | Descripción                                                      |
|------------------------|------------------------------------------------------------------|
| -a, --algorithm <name> | Nombre del algoritmo de esteganografía (listar con `--algorithms`) |

***

## Embed (Insertar mensaje)
...
**Opciones clave para `embed`:**

| Opción                                  | Descripción                                                                                           |
|-----------------------------------------|-------------------------------------------------------------------------------------------------------|
| -mf, --messagefile <archivo>            | Archivo de mensaje; usa `-` o omite para leer de stdin                                                |
| -cf, --coverfile <archivo(s)>           | Archivo(s) cover (acepta `*` y `?`, usar comillas); si coincide con varios, `-sf` se ignora           |
| -sf, --stegofile <archivo>              | Archivo de salida stego; omitido o `-` escribe en stdout                                               |
| -c, --compress                          | Comprimir mensaje antes de insertar (por defecto)                                                     |
| -C, --nocompress                        | No comprimir el mensaje                                                                               |
| -e, --encrypt                           | Encriptar mensaje antes de insertar                                                                   |
| -E, --noencrypt                         | No encriptar mensaje (por defecto)                                                                    |
| -p, --password <contraseña>             | Contraseña para encriptado; si se omite, solicita entrada                                               |
| -A, --cryptalgo <AES128|AES256>          | Algoritmo de cifrado; `AES256` requiere JCE ilimitado                                                   |

**Opciones adicionales con plugin RandomLSB:**

| Opción                                         | Descripción                                                          |
|------------------------------------------------|----------------------------------------------------------------------|
| -b, --maxBitsUsedPerChannel <n>                | Bits máximos por canal de color (1–7; default=3; mayor → menos calidad) |
| -cf (sin coverfile)                            | Genera imagen ruidosa automáticamente                               |
| -sf (sin stegofile)                            | Escribe salida PNG en stdout                                        |

***

## Extract (Extraer mensaje)
...
**Opciones clave para `extract`:**

| Opción                      | Descripción                                                      |
|-----------------------------|------------------------------------------------------------------|
| -sf, --stegofile <archivo>  | Archivo stego que contiene el mensaje                            |
| -xf, --extractfile <archivo>| Nombre de archivo extraído (opcional; sobrescribe nombre interno)|
| -xd, --extractdir <directorio> | Carpeta de destino (por defecto: directorio actual)            |
| -p, --password <contraseña> | Contraseña para desencriptado (si aplica)                       |

***

## Gensig (Generar firma)
...
**Opciones clave para `gensig`:**

| Opción                        | Descripción                                  |
|-------------------------------|----------------------------------------------|
| -gf, --sigfile <archivo>      | Archivo de firma de salida                   |
| -p, --password <contraseña>   | Contraseña para generar firma                |

***

## Embedmark (Insertar watermark)
...
**Opciones clave para `embedmark`:**

| Opción                          | Descripción                                                                |
|---------------------------------|----------------------------------------------------------------------------|
| -gf, --sigfile <archivo>        | Archivo de firma                                                           |
| -cf, --coverfile <archivo(s)>   | Archivo(s) cover (comodines permitidos; comillas necesarias)               |
| -sf, --stegofile <archivo>      | Archivo de salida watermarked                                              |

***

## Checkmark (Verificar watermark)
...
**Opciones clave para `checkmark`:**

| Opción                        | Descripción                         |
|-------------------------------|-------------------------------------|
| -sf, --stegofile <archivo>    | Archivo watermarked a verificar     |
| -gf, --sigfile <archivo>      | Archivo de firma para validación    |
...
- **Piping**  
  ```bash
  ls -R | java -jar openstego.jar embed -a randomlsb > stego.png
  ```
